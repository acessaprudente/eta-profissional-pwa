...
function balancoView() {
  ...
  const saved = carregarParametros ? carregarParametros('balanco') : null;
...
function calcularBalanco() {
  // leitura e validação
  const q = parseFloat(document.getElementById('bal_q').value);
  const c = parseFloat(document.getElementById('bal_c').value);
  ...
  const carga_mg_h = vazao_L_h * c;           // mg/h
  const carga_g_h = carga_mg_h / 1000;        // g/h
  const carga_kg_h = carga_g_h / 1000;        // kg/h
...
// função auxiliar para salvar parâmetros (usa storage.js se disponível)
function salvarBalanco() {
  ...
  if (typeof salvarParametros === 'function') {
    ...
    try { localStorage.setItem('eta_balanco', JSON.stringify(obj)); alert('Parâmetros salvos no localStorage.'); }
